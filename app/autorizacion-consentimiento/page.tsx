...
export default function AutorizacionConsentimientoPage() {
  ...
  const validate = () => {
    ...
    required.forEach(f => {
      if (!form[f as keyof typeof form]) newErrors[f] = "Campo requerido"
    })
  ...
  const tipoPaciente = typeof window !== "undefined" ? localStorage.getItem("tipoPaciente") : null
  const nextRoute = tipoPaciente === "bancario" ? "/contrato-bancario" : "/consentimiento"
  const nextLabel = tipoPaciente === "bancario" ? "Siguiente: Contrato Bancario" : "Siguiente: Consentimiento"
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">El PDF de Autorización con Consentimiento ha sido descargado exitosamente.</p>
          <div className="flex gap-3 justify-center">
            <Button onClick={() => setShowSuccess(false)} variant="outline">Regresar al Formulario</Button>
            <Link href={nextRoute}>
              <Button className="bg-medical-teal hover:bg-medical-teal/90 text-white">{nextLabel}</Button>
            </Link>
          </div>
        </motion.div>
      </div>
  ...
  const Field = ({ label, name, type = "text", className = "", placeholder = "" }: { label?: string; name: string; type?: string; className?: string; placeholder?: string }) => (
    <div className={`flex items-end gap-1 ${className}`}>
      {label && <span className="text-[11px] font-bold uppercase whitespace-nowrap leading-tight">{label}:</span>}
      <div className="flex-1 flex flex-col">
        <input
          ...
          className={`border-b border-black bg-transparent text-[11px] px-0.5 py-0 outline-none min-w-0 w-full ${errors[name] ? "border-red-500" : ""}`}
  ...
  const FieldFixed = ({ label, name, width = "flex-1", placeholder = "" }: { label?: string; name: string; width?: string; placeholder?: string }) => (
    <div className={`flex items-end gap-1 ${width}`}>
      ...
      <div className="flex flex-col flex-1">
        <input
          ...
          onChange={e => handleChange(name, e.target.value)}
          placeholder={placeholder}
          className={`border-b border-black bg-transparent text-[11px] px-0.5 py-0 outline-none w-full ${errors[name] ? "border-red-500" : ""}`}
          style={{ lineHeight: "1.4" }}
        />
        {errors[name] && <span className="text-[9px] text-red-500">{errors[name]}</span>}
      </div>
  ...
  // Helper inline — línea con label + underline editable
  const Line = ({ label, name, w = "flex-1", placeholder = "" }: { label?: string; name: string; w?: string; placeholder?: string }) => (
    <span className={`inline-flex items-end gap-0.5 ${w}`}>
      {label && <span className="text-[11px] font-bold uppercase whitespace-nowrap shrink-0">{label}</span>}
      <span className="flex-1 flex flex-col min-w-0">
        <input
          ...
          onChange={e => handleChange(name, e.target.value)}
          placeholder={placeholder}
          className={`border-b border-black bg-transparent text-[11px] px-0.5 outline-none w-full ${errors[name] ? "border-red-500" : ""}`}
          style={{ lineHeight: "1.5" }}
        />
        {errors[name] && <span className="text-[8px] text-red-500 leading-none">{errors[name]}</span>}
      </span>
    </span>
  )
  ...
  return (
    <div className="min-h-screen bg-gray-200 p-4 light">
      {/* Header buttons — outside printable area */}
      <div className="max-w-[816px] mx-auto mb-3 flex justify-between items-center">
        ...
        <Button
          ...
          className="bg-medical-teal hover:bg-medical-teal/90 text-white flex items-center gap-2"
        >
        ...
        >
          ...
          <div style={{ border: "2px solid black", marginBottom: 12 }}>
            ...
            <div style={{ padding: "10px 14px" }}>
              ...
              {/* Advertencia */}
              <p style={{ fontSize: 10.5, textAlign: "center", marginBottom: 8, lineHeight: 1.4, color: "#cc0000", fontWeight: "bold" }}>
              ...
              {/* Fila 1: NOMBRE DEL COLABORADOR + FECHA */}
              ...
              <div style={{ border: "1px solid black", padding: "6px 8px", marginBottom: 6 }}>
                ...
                {/* TRATAMIENTO línea 1 + RS (checkbox small) */}
                ...
                <div style={{ borderBottom: "1px solid black", marginBottom: 4, minHeight: 18 }}>
                  <input
                    type="text"
                    value={form.tratamiento2}
                    onChange={e => handleChange("tratamiento2", e.target.value)}
                ...
                <div style={{ borderBottom: "1px solid black", marginBottom: 6, minHeight: 18 }}>
                  <input
                    type="text"
                    value={form.tratamiento3}
                    onChange={e => handleChange("tratamiento3", e.target.value)}
                    style={{ width: "100%", background: "transparent", fontSize: 11, outline: "none", padding: "0 2px" }}
                  />
                </div>

                {/* COSTO TOTAL / PAGO SEMANAL / NO. SEMANAS */}
                <div style={{ display: "flex", gap: 8, marginBottom: 5, alignItems: "flex-end" }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                  ...
                  <input
                    type="text"
                    value={form.personaDesignada}
                    onChange={e => handleChange("personaDesignada", e.target.value)}
              ...
              <p style={{ fontSize: 10.5, lineHeight: 1.55, marginBottom: 6 }}>
                ...
                />{" "}
                M.N),{" "}
                <strong style={{ color: "#cc0000", fontStyle: "italic" }}>
                  ESTOY DE ACUERDO QUE NO HABRA CANCELACION ALGUNA DE TRATAMIENTOS DENTALES Y MUCHO MENOS REMBOLSO
                  MONETARIOS PARCIALES O TOTALES, SOLAMENTE PODRE TRANSFERIR MI SALDO EN TRATAMIENTO A OTRA PERSONA
                  QUE YO DESIGNE.
                </strong>{" "}
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 6, marginTop: 6 }}>
                <Checkbox
                  ...
                  checked={form.aceptaAutorizacion}
            ...
            <div style={{ padding: "10px 14px" }}>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "flex-end", marginBottom: 8 }}>
                ...
                <span style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                  <select
                    ...
                    onChange={e => handleChange("medicoTratante", e.target.value)}
              ...
              <div style={{ display: "flex", gap: 32, alignItems: "flex-end", marginBottom: 8 }}>
                <span style={{ display: "flex", gap: 4, alignItems: "flex-end", flex: 1 }}>
                  ...
                  <span style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                    <input
                      type="text"
                      value={form.nombrePacienteConsentimiento}
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "flex-end", fontSize: 10.5, marginBottom: 8 }}>
                ...
                <span style={{ display: "flex", flexDirection: "column" }}>
                  <input
                    type="text"
                    value={form.diaAutorizacion}
                    onChange={e => handleChange("diaAutorizacion", e.target.value)}
                    placeholder="DD"
                    className={errors.diaAutorizacion ? "border-red-500" : ""}
                ...
                <span style={{ display: "flex", flexDirection: "column" }}>
                  <input
                    type="text"
                    value={form.mesAutorizacion}
                    onChange={e => handleChange("mesAutorizacion", e.target.value)}
                    placeholder="MES"
                    className={errors.mesAutorizacion ? "border-red-500" : ""}
                ...
                <span style={{ display: "flex", flexDirection: "column" }}>
                  <input
                    type="text"
                    value={form.anioAutorizacion}
                    onChange={e => handleChange("anioAutorizacion", e.target.value)}
                    placeholder="AAAA"
                    className={errors.anioAutorizacion ? "border-red-500" : ""}
              ...
              {/* Cláusula no cancelación + línea firma */}
              <p style={{ fontSize: 10.5, lineHeight: 1.55, marginBottom: 6, color: "#cc0000", fontWeight: "bold" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
                <Checkbox
                  id="aceptaNoCancelacion"
                  checked={form.aceptaNoCancelacion}
